...
function makeRef() {
  return 'RES-' + String(Math.floor(Math.random() * 99999)).padStart(5, '0');
}
...
router.get('/slots', async (req, res) => {
  try {
    ...
    // Get max covers setting
    const [[setting]] = await db.query(
      "SELECT `value` FROM settings WHERE `key` = 'max_covers_per_slot'"
    );
    const maxCovers = parseInt(setting?.value || 20);

    // Get booked counts per slot for this date
    ...
    // Build time slots 8am–10pm in 30-min increments
    const slots = [];
    for (let h = 8; h <= 21; h++) {
      for (let m of [0, 30]) {
...
router.post('/', async (req, res) => {
  const { guest_name, phone, party_size, res_date, res_time, food_choice, notes } = req.body;
  if (!guest_name || !party_size || !res_date || !res_time) {
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO reservations (ref, guest_name, phone, party_size, res_date, res_time, food_choice, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [ref, guest_name, phone || null, party_size, res_date, res_time, food_choice || null, notes || null]
    );
...
// GET /api/reservations/settings — get capacity setting
